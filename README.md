...
- A built and installed [`mirage`](../mirage) **v1.2.0 or later** (the `find_package(Mirage CONFIG)` package under `mirage/install/`) — this plugin uses v1.2.0 APIs (`Camera::fStop`/`focalLength`/`sensorWidth`/`sensorHeight`, `Scene::lights`/`Mirage::PunctualLight`, `Primitive::eRect`, `Skylight.h`'s `BakePreethamSky`, `Texture::GenerateMips`/`udimGridWidth`/`udimGridHeight`) on top of v1.1.0's (`Material::opacity`/`normalTextureIndex`, `Mesh::verticesEnd`/`tangents`, `Scene::AddInstancer`), none of which exist in v1.0.0.
